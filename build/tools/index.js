...
function registerReadFromMemoryTool() {
    server.tool("read_from_memory", "Reads the user's current CSS knowledge from memory.", {}, // No input parameters required for this tool
    async () => {
        try {
...
function registerWriteToMemoryTool() {
    ...
    server.tool("write_to_memory", "Updates the user's CSS knowledge memory for a specific concept.", writeMemoryInputShape, // Pass the shape to define the input schema for the client
    async (args) => {
